...
@debug
def writeFeatures(costFeature, model):
    """
    ...
    with open(config.path(path,"features",  "importances", "feature_importance_%s.txt" % (d.tags[costFeature])),'wb')as f:
        for feature, importance in sortedFeatures:
            write = "%s#%f\n" % (d.tags[feature], importance)
            f.write(write.replace("#", (24 - len(write)) * " "))
    return costFeature, [tag for tag,importance in sortedFeatures]


@debug
def costModel(x_train, y_train, costIndex = 0):
    """
    Creates and fits the model based on x_train and y_train
    Returns model as specified in import
    """
    model = Model()
    model.fit(x_train, y_train[:,costIndex])
...
def main():
    ...
    data = np.genfromtxt(config.path(path, datafile.lower() + ".csv"), delimiter=",")
    costs = config.get(config.path(path, "formatted", "formatCost.p"), ff.getCosts, data = data, costs = d.costs)
    ...
    for costIndex in xrange(len(d.costs)):
        ...
        model = config.get(config.path(path,"models", "model_%s.p" % d.costs[costIndex]), costModel, x_train = x_train, y_train = y_train, costIndex = costIndex)

        #Sorting and Writing Important Features
        feat_indices = config.get(config.path(path,"features", "features%s.p" % d.tags[d.costs[costIndex]]), writeFeatures, costFeature = d.costs[costIndex], model = model)

        #Testing model accuracies
        predictions = model.predict(x_test)
        accuracy = score(predictions, y_test[:,costIndex])
        moneyError.append((d.tags[d.costs[costIndex]],accuracy ** .5))
    print moneyError
